...
def get_muon_param_groups(named_params, adamw_lr=3e-4, adamw_betas=(0.9, 0.95), adamw_eps=1e-8, adamw_wd=0.0, muon_lr=0.02, muon_momentum=0.95, muon_wd=0.2, muon_ns_steps=5, verbose=False):
    ...
    for name, p in named_params:
        ...
        is_fp_head = bool(getattr(p, '_fp_head', False))
        fp_scale = float(getattr(p, '_fp_lr_scale', 1.0)) if is_fp_head else 1.0
        no_wd = bool(getattr(p, '_no_muon_wd', False) or getattr(p, '_no_weight_decay', False))
        force_wd = getattr(p, '_fp_force_wd', None)
        if p.ndim < 2 or is_embed or is_norm or is_parcae_recurrent or is_fp_head:
            if force_wd is not None:
                eff_wd = float(force_wd)
            elif no_wd:
                eff_wd = 0.0
            else:
                eff_wd = adamw_wd
            adamw_groups.setdefault((fp_scale, eff_wd), []).append(p)
            if verbose:
                print(f"AdamW: {name} (fp_scale={fp_scale}, wd={eff_wd})")
        else:
            key = (p.shape, fp_scale, no_wd)
            muon_groups.setdefault(key, []).append(p)
            if verbose:
                print(f"Muon: {name} {p.shape} (fp_scale={fp_scale}, wd={'0' if no_wd else str(muon_wd)})")
    groups = []
    for (fp_scale, eff_wd), params in adamw_groups.items():
        groups.append({'params': params, 'kind': 'adamw',
                       'lr': adamw_lr * fp_scale, 'base_lr': 1.0 * fp_scale,
                       'betas': adamw_betas, 'eps': adamw_eps,
                       'weight_decay': float(eff_wd)})
    for (shape, fp_scale, no_wd), params in muon_groups.items():
        groups.append({'params': params, 'kind': 'muon',
                       'lr': muon_lr * fp_scale,
                       'base_lr': (muon_lr / adamw_lr) * fp_scale,
                       'momentum': muon_momentum,
                       'weight_decay': 0.0 if no_wd else muon_wd,
                       # Tells train.py to leave this group's wd alone
                       # instead of overriding it via the global tapering
                       # schedule on every optimizer step.
                       'pin_wd': no_wd,
